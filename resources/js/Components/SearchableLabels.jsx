...
export default function SearchableLabels({ selectedLabels = [], onChange }) {
    ...
    const loadLabels = async () => {
        ...
        setError(null);
        try {
            const response = await axios.get(`/line-items/available-labels?search=${encodeURIComponent(query)}`);
            if (response.data.status === 'success') {
                // Limit to 10 results
                setLabels(response.data.labels.slice(0, 10));
            } else {
                throw new Error(response.data.message || 'Failed to load labels');
            }
        } catch (err) {
    ...
    const handleSelect = (label) => {
        ...
        if (isSelected) {
            newLabels = selectedLabels.filter(selected => selected.id !== label.id);
        } else {
    ...
    return (
        <div className="relative w-full">
            <Combobox value={selectedLabels} onChange={onChange} multiple>
